          ...
          <div className="card" style={{ padding: 24 }}>
            ...
            {mode === "week" && (
              <>
                ...
                <div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 200, overflowX: "auto" }}>
                  {weeks.map((x, i) => {
                    const here = i === wIdx;
                    return (
                      <button
                        ...
                        title={`${x.w}: ${fmt(x.v)}`}
                        style={{ flex: "0 0 auto", minWidth: 28, display: "flex", flexDirection: "column", alignItems: "center", gap: 7, justifyContent: "flex-end", background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: "var(--sans)" }}
                      >
                        <div style={{ width: 22, height: `${((x.v / maxW) * 150).toFixed(0)}px`, background: "var(--neg)", borderRadius: "4px 4px 0 0", opacity: here ? 0.95 : 0.4, transition: "opacity .12s" }} />
                        <span className="axis" style={{ fontFamily: "var(--mono)", fontSize: 9, color: here ? "var(--ink)" : "var(--muted)", fontWeight: here ? 600 : 400, writingMode: "vertical-rl", transform: "rotate(180deg)", height: 44 }}>
                          {x.w}
                        </span>
                      </button>
                    );
            ...
            {mode === "month" ? (
              ...
              <>
                ...
                {weekCats.length === 0 ? (
                  ...
                  weekCats.map((cat) => (
                    <div key={cat.name} style={{ display: "flex", alignItems: "center", gap: 12, padding: "9px 0" }}>
                      <span style={{ flex: "none", width: 12 }} />
                      <span style={{ flex: 1, fontSize: 12.5, color: "var(--ink-2)" }}>{cat.name}</span>
                      <div style={{ flex: "none", width: 70, height: 8, borderRadius: 5, background: "var(--hair-2)", overflow: "hidden" }}>
